...
def find_executable(cmd: str) -> Optional[str]:
    ...
    for directory in path_dirs:
        if not os.path.isdir(
            directory
        ):  # normally only directories are present in PATH, but just in case if someone has put a file in PATH, then ignore it
            continue
...
def change_directory(input: str) -> None:
    if input == "~":
        input = os.path.expanduser(
            "~"
        )  # standard notation for home dir , so gives absolute path for home , in every OS
    try:
        os.chdir(input)
    except OSError:
...
def auto_completer(text: str, state: int) -> Optional[str]:
    ...
    ]  # taking all the possible keys in builtin_commands
    executable_options = [
        exe for exe in executables if exe.startswith(text)
    ]  # taking all the possible executable commands , from filepaths in PATH
    options = builtin_options + executable_options  # total options to choose from
    if state < len(options):
        return (
            options[state] + " "
        )  # cursor appears after completion and an extra space
    return None
...
def parse_input(command: str) -> list[str]:
    ...
    lexer = shlex.shlex(
        command, posix=True
    )  # posix=True tells to read the command like how Unix shell would read, if False then its like a normal parser
...
def extract_redirections(
    tokens: list[str],
) -> tuple[list[str], Optional[str], str, Optional[str], str]:
    command: list[str] = []
    ...
    while i < len(tokens):
        ...
        if token in [
            ">",
            "1>",
        ]:  # file descriptor 1, for sending output to output file, write mode
            stdout_target = tokens[i + 1]
            stdout_mode = "w"
            i += 2
        elif (
            token == "2>"
        ):  # file descriptor 2, for sending errors to output file, write mode
            ...
            ">>",
            "1>>",
        ]:  # file descriptor 1, for sending output to output file, append mode
            stdout_target = tokens[i + 1]
            stdout_mode = "a"
            i += 2
        elif (
            token == "2>>"
        ):  # file descriptor 2, for sending errors to output file, append mode
            stderr_target = tokens[i + 1]
            stderr_mode = "a"
            i += 2
        else:
...
def get_history(input: str) -> None:
    ...
    if (
        len(args) == 2 and args[0] == "-r"
    ):  # read the commands in the specified path , and them to history too
        file_path = args[1]
        try:
    ...
    for i, tokens in enumerate(stages):
        ...
        if pid == 0:  # child process
            if prev_read_fd:
                os.dup2(
                    prev_read_fd, 0
                )  # make it stdin(fd=0) for input from previous process
                os.close(
                    prev_read_fd
                )  # closing the original pipe since its not needed now
            ...
            if w and r:
                ...
                os.close(
                    w
                )  # closing the original write end of the pipe now , since now its assigned to stdout
                os.close(
                    r
                )  # closing the original read since the child does not read anything from here
            ...
            if tokens[0] in builtin_commands:
                ...
                os.execvp(
                    tokens[0], tokens
                )  # replaces the whole child process , with this process
            ...
            prev_read_fd = (
                r  # so that the next process can read from this read end of the pipe
            )
...
def handle_command(command: str) -> None:
    ...
    commandArr, stdout_target, stdout_mode, stderr_target, stderr_mode = (
        extract_redirections(tokens)
    )
    ...
    if commandArr[0] in builtin_commands:
        ...
        if stdout_target and stderr_target:
            with open(stdout_target, stdout_mode) as stdout_file, open(
                stderr_target, stderr_mode
            ) as stderr_file:
    ...
    if executable_path:
        ...
        if stdout_target and stderr_target:
            ...
            with open(stdout_target, stdout_mode) as stdout_file:
                subprocess.run(
                    commandArr, executable=executable_path, stdout=stdout_file
                )
        elif stderr_target:
            with open(stderr_target, stderr_mode) as stderr_file:
                subprocess.run(
                    commandArr, executable=executable_path, stderr=stderr_file
                )
        else:
